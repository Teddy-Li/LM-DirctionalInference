...
if __name__ == '__main__':
    ...
    assert args.range in ['all', 'hypo_only']
    ...
    if args.range in ['all']:
        ...
        dataloader = load_custom_data(args, args.model, model, '../datasets/data_en_levy_holt/levy_holt/dev.txt')
    ...
    if args.range in ['all']:
        ...
        dataloader = load_custom_data(args, args.model, model, '../datasets/data_en_sherliic/levy_holt/dev.txt')
        trainer = pl.Trainer(gpus=args.gpus, logger=False)
        trainer.test(model, test_dataloaders=dataloader)
    ...
    if args.range in ['all']:
        ...
        dataloader = load_custom_data(args, args.model, model, '../datasets/data_levyholt_reparse_reparse/levy_holt/dev.txt')
        trainer = pl.Trainer(gpus=args.gpus, logger=False)
        trainer.test(model, test_dataloaders=dataloader)
    ...
    if args.range in ['all']:
        ...
        dataloader = load_custom_data(args, args.model, model, '../datasets/data_levyholt_raw_raw/levy_holt/dev.txt')
        trainer = pl.Trainer(gpus=args.gpus, logger=False)
        trainer.test(model, test_dataloaders=dataloader)
    ...
    if args.range in ['all']:
        ...
        dataloader = load_custom_data(args, args.model, model, '../datasets/data_sherliic_raw_raw/levy_holt/dev.txt')
        trainer = pl.Trainer(gpus=args.gpus, logger=False)
        trainer.test(model, test_dataloaders=dataloader)
    ...
    if args.range in ['all']:
        ...
        dataloader = load_custom_data(args, args.model, model, '../datasets/data_endir_levyholt/levy_holt/dev.txt')
        trainer = pl.Trainer(gpus=args.gpus, logger=False)
        trainer.test(model, test_dataloaders=dataloader)
    ...
    if args.range in ['all']:
        ...
        dataloader = load_custom_data(args, args.model, model, '../datasets/data_zhdir_levyholt_raw/levy_holt/dev.txt')
        trainer = pl.Trainer(gpus=args.gpus, logger=False)
        trainer.test(model, test_dataloaders=dataloader)
    ...
    if args.range in ['all']:
        ...
        dataloader = load_custom_data(args, args.model, model, '../datasets/data_zhdir_levyholt_rep/levy_holt/dev.txt')
        trainer = pl.Trainer(gpus=args.gpus, logger=False)
        trainer.test(model, test_dataloaders=dataloader)
    ...
    if args.range in ['hypo_only']:
        print(f"Testing on: '../datasets/data_endir_levyholt/hypo_only/dev.txt'")
        model.set_minimum_precision(0.5)
        model.set_pr_rec_curve_path(os.path.join(ckpt_root, 'endir_levyholt_hypoonly_dev_pr_rec.txt'))
        model.set_score_outfile(os.path.join(ckpt_root, 'endir_levyholt_hypoonly_dev_Y.txt'))
        dataloader = load_custom_data(args, args.model, model, '../datasets/data_endir_levyholt/hypo_only/dev.txt')
        trainer = pl.Trainer(gpus=args.gpus, logger=False)
        trainer.test(model, test_dataloaders=dataloader)
    ...
    if args.range in ['hypo_only']:
        print(f"Testing on: '../datasets/data_en_levy_holt/hypo_only/dev.txt'")
        model.set_minimum_precision(0.19763)
        model.set_pr_rec_curve_path(os.path.join(ckpt_root, 'en_levyholt_hypoonly_dev_pr_rec.txt'))
        model.set_score_outfile(os.path.join(ckpt_root, 'en_levyholt_hypoonly_dev_Y.txt'))
        dataloader = load_custom_data(args, args.model, model, '../datasets/data_en_levy_holt/hypo_only/dev.txt')
        trainer = pl.Trainer(gpus=args.gpus, logger=False)
        trainer.test(model, test_dataloaders=dataloader)
