...
def main(args: argparse.Namespace):
    rel_idx = {}
    with open(args.relation_index, 'r', encoding='utf8') as f:
        for line in f:
            idx, rel = line.strip().split('\t')
            rel_idx[idx] = rel

    pos_pairs = []
    with open(args.sherliic_file, 'r', encoding='utf8') as f:
        r = csv.reader(f)
        next(r)  # headers
        for row in r:
            cls = row[17] == 'yes'
    ...
    with open(args.scored_pattern_file, 'w', encoding='utf8') as fout:
