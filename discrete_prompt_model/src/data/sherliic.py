...
import re
from .common import PREM_KEY, HYPO_KEY, LABEL_KEY, SENT_KEY, ANTI_KEY, LANGUAGE_KEY,\
    MASKED_SENT_KEY, MASKED_ANTI_KEY, PATTERNS, PATTERNS_GERMAN, NEGATION_NECESSARY, ANTIPATTERNS, ANTIPATTERNS_GERMAN,\
...
def unpack_row(row: Sequence[str])\
    ...
    return int(sample_id), int(prem_type), int(prem_rel), int(hypo_type), int(hypo_rel),\
        ...
        float(esr_score), int(num_disagr), language_tag
...
class SherliicBase(Dataset):
    ...
    def load_dataset(self):
        ...
        with open(self.fn, 'r', encoding='utf8') as f:
            cr = csv.reader(f)
            next(cr)  # headers
    ...
    def create_instances(
            ...
            rel_score: float, sign_score: float, esr_score: float, num_disagr: int, language_tag: str
...
class SherliicPattern(SherliicBase):
    def __init__(self, path_to_csv: str, tokenizer: Optional[PreTrainedTokenizer] = None,
        ...
        if pattern_file is None:
            ...
            self.patterns_de = PATTERNS_GERMAN
    ...
    def create_single_instance(self, premise: Tuple[str, str, str, str],
                               ...
                               examples_B: Tuple[str, str, str], gold_label: bool, language_tag: str
        ...
        if self.pattern_idx < 0:
            ...
            for pat, do_neg in zip(patterns, self.negation_necessary):
        ...
        if self.antipattern_idx < 0:
            ...
            for pat, do_neg in zip(antipatterns, self.anti_negation_necessary):
